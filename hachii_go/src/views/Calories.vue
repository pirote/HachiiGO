        ...
        <tbody>
          <tr v-for="item in resultQuery" :key="item.id">
            ...
            <td style="text-align: right;">
              ...
              <a v-if="item.status === 'favorite'" target="_blank" style="color: #008cff;">
                <svg
                  id="Capa_1"
                  enable-background="new 0 0 512 512"
                  height="18"
...
export default {
  ...
  updated() {
    ...
    for (var i = 0; i < this.dataFavorite.length; i++) {
      for (var j = 0; j < this.dataAll.length; j++) {
        if (this.dataFavorite[i].Food === this.dataAll[j].Food) {
          this.dataAll[j]["status"] = "favorite";
        }
      }
    }
  },
  ...
  methods: {
    ...
    async Foods() {
      ...
      await dataRef.on("child_added", (snapshot) => {
        this.dataFoods.push({ ...snapshot.val(), status: "" });
      });
    },
    async Sweet() {
      var dataRef = database.ref("/FoodData/Sweet");
      await dataRef.on("child_added", (snapshot) => {
        this.dataSweet.push({ ...snapshot.val(), status: "" });
      });
    },
    async Meats() {
      var dataRef = database.ref("/FoodData/Meats");
      await dataRef.on("child_added", (snapshot) => {
        this.dataMeats.push({ ...snapshot.val(), status: "" });
      });
    },
    async Drink() {
      var dataRef = database.ref("/FoodData/Drink");
      await dataRef.on("child_added", (snapshot) => {
        this.dataDrink.push({ ...snapshot.val(), status: "" });
      });
    },
    async MixedFruit() {
      var dataRef = database.ref("/FoodData/MixedFruit");
      await dataRef.on("child_added", (snapshot) => {
        this.dataMixedFruit.push({ ...snapshot.val(), status: "" });
      });
    },
    async Store() {
      var dataRef = database.ref("/FoodData/Store");
      await dataRef.on("child_added", (snapshot) => {
        this.dataStore.push({ ...snapshot.val(), status: "" });
      });
    },
    async Other() {
      var dataRef = database.ref("/FoodData/Others");
      await dataRef.on("child_added", (snapshot) => {
        this.dataOther.push({ ...snapshot.val(), status: "" });
      });
    },
    async DataWait() {
      var dataRef = database.ref("/FoodData/DataWaitConfirm");
      await dataRef.on("child_added", (snapshot) => {
        this.dataWait.push({ ...snapshot.val(), status: "" });
      });
    },
